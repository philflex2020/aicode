...
//I have an any value and a io_point_struct
...
struct cfg_struct {
    ...
    int offset;
    int type;
    std::vector<std::shared_ptr<io_point_struct>> items;
};
...
/// @param myCfg 
/// @return 
u64 gcom_decode_any(u16* raw16, u8*raw8, struct io_point_struct* item, std::any& output, struct cfg& myCfg)
{
...
void decode_bval(bool bval, std::shared_ptr<io_point_struct> item, std::stringstream &ss, cfg_struct& cfg) {
    ss << addQuote(item->id) << ":";
...
void decode_bval_from_index(std::any& value, std::shared_ptr<io_point_struct> item, std::stringstream &ss, cfg_struct& cfg) {
    u64 val = getAnyVal(value, (u64)0) ;
    auto bval = ((val >> item->bit_index) & 1UL) == 1UL;//std::any_cast<bool>(input);
    decode_bval(bval, item, ss, cfg);
}        

void decode_bval_from_value(std::any& value, std::shared_ptr<io_point_struct> item, std::stringstream &ss, cfg_struct& cfg) {
    u64 val = getAnyVal(value, (u64)0) ;
    auto bval = (val != 0);
    decode_bval(bval, item, ss, cfg);
}        
...
void decode_bit_field(std::any &value, std::shared_ptr<io_point_struct> item, std::stringstream &bss, cfg_struct& cfg) {
    u64 val = getAnyVal(value, (u64)0) ;
    bool firstOne = true;
...
void decode_enum(std::any &value, std::shared_ptr<io_point_struct> item, std::stringstream &bss, cfg_struct& cfg) {
...
bool modbus_decode(u16 *raw16, std::shared_ptr<io_point_struct> item, std::any& value, std::stringstream &ss, cfg_struct& cfg);
...
int main() {
    for ( int i = 0 ; i < 4 ; ++i) {
        auto item = std::make_shared<io_point_struct>();
