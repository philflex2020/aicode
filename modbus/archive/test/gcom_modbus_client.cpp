        ...
        if (method == Method_Types::Set)
        {
            ...
            if (base_uri_info.decode_idx == Decode_All_Idx) // multi-set
            {
                ...
                for (auto pair : set_obj)
                {
                    ...
                    // reg type check:
                    if (inner_uri_info.register_type == Register_Types::Input || inner_uri_info.register_type == Register_Types::Discrete_Input)
                    {
                    ...
                    if (inner_uri_info.register_type == Register_Types::Coil || inner_uri_info.flags.is_individual_bit())
                    {
            ...
            {
                // reg type check:
                if (base_uri_info.register_type == Register_Types::Input || base_uri_info.register_type == Register_Types::Discrete_Input)
                {
                ...
                // check for true/false and 1/0 for coils and individual_bits:
                if (base_uri_info.register_type == Register_Types::Coil || base_uri_info.flags.is_individual_bit()) {
                    if (!to_set.holds_uint() || !(to_set.get_uint_unsafe() == 1UL || to_set.get_uint_unsafe() == 0UL)) {
    ...
    while (my_workspace.keep_running) {
        ...
        if (work_qs.set_q.try_pop(set_work)) {
            ...
            for (auto& val : set_work.set_vals) {
                ...
                switch(map_workspace.register_type) {
        ...
        // we need to find register_type, start_offset and num_regs its that simple
        // or it should be.
        ...
        if (work_qs.poll_q.try_pop(poll_work)) {
            ...
            if (1) printf(" ---->>>> >>> configs start_offset %d\n", (int)configs_array.configs[0].components[pub_work.comp_idx].register_groups[pub_work.thread_idx].start_offset);
    ...
    */
    ...
    while (client_workspace.keep_running) {
        ...
        if (work_qs.pub_q.try_pop(pub_work)) { // IO_threads produce onto this for main to consume
            ...
            auto print_poll_if_finished = [&]() {
                ...
                if (--comp_workspace.num_polls_in_q_left == 0) {
                    ...
                    for (u8 thread_idx = 0; thread_idx < comp_workspace.num_threads; ++thread_idx)
                    {
                        ...
                        if (curr_decoded_cache.register_type == Register_Types::Holding || curr_decoded_cache.register_type == Register_Types::Input)
                        {
            ...
            switch(decoded_cache.register_type) {
                case Register_Types::Holding: // fallthrough
                case Register_Types::Input: {
...
bool initialize_arena_from_config(const config_loader::Client_Configs_Array& configs_array, Main_Workspace& main_workspace)
{
    ...
    for (const auto& config : configs_array.configs)
    {
        ...
        for (const auto& comp : config.components)
        {
            ...
            for (const auto& reg_map : comp.register_groups)
            {
...
bool allocate_main_workspace_from_config(const config_loader::Client_Configs_Array& configs_array, Main_Workspace& main_workspace)
{
    ...
    for (u8 client_idx = 0; client_idx < main_workspace.num_clients; ++client_idx)
    {
        ...
        {
            // if (!main_workspace.arena.allocate(curr_client_workspace.msg_staging_areas[comp_idx].set_msgs, config.components[comp_idx].register_groups.size())) return false;
            // curr_client_workspace.msg_staging_areas[comp_idx].num_threads = static_cast<u8>(config.components[comp_idx].register_groups.size());
        }
        ...
        for (u8 comp_idx = 0; comp_idx < curr_client_workspace.num_comps; ++comp_idx)
        {
            ...
            if (!main_workspace.arena.allocate(curr_comp_workspace.decoded_caches, curr_config_comp.register_groups.size())) return false;
            curr_comp_workspace.num_threads = static_cast<u8>(curr_config_comp.register_groups.size());
            ...
            {
                const auto& curr_config_reg_map = curr_config_comp.register_groups[thread_idx];

                auto& curr_decoded_cache = curr_comp_workspace.decoded_caches[thread_idx];
                if (curr_config_reg_map.register_type == Register_Types::Holding || curr_config_reg_map.register_type == Register_Types::Input)
                {
            ...
            for (u8 thread_idx = 0; thread_idx < curr_comp_workspace.num_threads; ++thread_idx)
            {
                ...
                if (curr_config_reg_map.register_type == Register_Types::Holding || curr_config_reg_map.register_type == Register_Types::Input)
                {
                    ...
                    if (curr_config_reg_map.register_type == Register_Types::Holding && curr_config_reg_map.num_bit_str_arrays > 0)
                    {
                ...
                // {
                //     if (curr_config_reg_map.register_type == Register_Types::Holding || curr_config_reg_map.register_type == Register_Types::Coil)
                //     {
                //         if (!main_workspace.arena.allocate(curr_thread_workspace.debounce_last_set_times, curr_config_reg_map.map.size())) return false;
                //         if (curr_config_reg_map.register_type == Register_Types::Holding)
                //         {
    ...
    for (u8 client_idx = 0; client_idx < main_workspace.num_clients; ++client_idx)
    {
        ...
        for (u8 comp_idx = 0; comp_idx < curr_client_workspace.num_comps; ++comp_idx)
        {
            ...
            for (u8 thread_idx = 0; thread_idx < curr_comp_workspace.num_threads; ++thread_idx)
            {
                ...
                curr_decoded_cache.register_type = curr_config_reg_map.register_type;
                ...
                curr_thread_workspace.register_type      = curr_config_reg_map.register_type;
                ...
                for (u8 decode_idx = 0; decode_idx < curr_thread_workspace.num_decode; ++decode_idx)
                {
                    ...
                    if (curr_config_decode.individual_bits) // put individual_bits uris into the uri_map:
                    {
                        ...
                        for (const auto& bit_str : curr_config_decode.compressed_bit_strings)
                        {
                            ...
                            curr_decode_uri_info.register_type = curr_config_reg_map.register_type;
...
//void test_iothread(const char *ip, int port, const char *oper, int device_id, const char *register_type, int offset, int value, bool debug);
...
void pubCallback(TimeObject* t, void* p) {
    ...
    // we have the coms so find the register groups
    //we have to create an io_work structure.
    // if the register is enabled.

    

    for ( auto &myreg : mypub->comp->register_groups)
    {
        std::cout 
        << " comp " << mypub->comp->id 
...
void sortMapsVector(std::vector<cfg::io_point_struct>& elements) {
    std::sort(elements.begin(), elements.end(), cfg::io_point_struct::compare);
}

// skips packed registers
void checkRegGroupSize(std::vector<cfg::io_point_struct>& elements) {
...
int getFirstOffset(const std::vector<cfg::io_point_struct>& elements) {
    return elements[0].offset;
}

// skips packed registers
// TODO need to set up the next value correctly
int checkMapsOffsets(std::vector<cfg::io_point_struct>& elements) {
...
int main(const int argc, const char* argv[]) noexcept {
    ...
    if(cmd == "test_cfg_uri")
    {
        ...
        if(cmd == "test_io_cfg")
        {
            ...
            for (auto &mycomp : myCfg.components)
            {
                ...
                for (auto &myreg : mycomp.register_groups)
                {
                    checkRegGroupSize(myreg.io_point_map);
                    // TODO add next pointers
                    sortMapsVector(myreg.io_point_map);


                    int first_offset = getFirstOffset(myreg.io_point_map);
                    int total_size = checkMapsOffsets(myreg.io_point_map);
                    ...
                    for (auto &mymap : myreg.io_point_map)
                    {
    ...
    if (argc > 3)
    {
        ...
        if(cmd == "test_iopoint")
        {
            ...
            test_io_point(argv[2], atoi(argv[3]), oper, device_id, register_type, offset, value, debug);
