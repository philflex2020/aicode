...
void test_io_point_multi(const char* ip, int port, int timeout, const char *oper, int device_id, const char *regtype, int offset, int value, 
                   int num_threads, struct cfg&, bool debug);
void test_io_point_single(const char* ip, int port, int timeout, const char *oper, int device_id, const char *regtype, int offset, int num_regs, int value,
                   int num_threads,  struct cfg&, bool debug);
...
void startRespThread(struct cfg&);
void stopRespThread(struct cfg&);
...
int test_io_threads(struct cfg&);
...
int  gcom_setup_pubs(std::map<std::string, std::any>& gcom_map, struct cfg& myCfg, double wait_time,  bool debug);
...
// std::shared_ptr<cfg::map_struct> test_findMapVar(
//     const struct cfg &myCfg, const std::vector<std::string>& uri_keys) {

//     if (uri_keys.size() < 3 || uri_keys[0].empty()) return nullptr;

//     try {
//         // Using at() for safe access. Catch exceptions if key not found
//         auto &myComp = myCfg.itemMap.at(uri_keys[0]);
//         auto &myReg = myComp.at(uri_keys[1]);
//         return myReg.at(uri_keys[2]);
//     } catch (const std::out_of_range&) {
//         // Key not found, return nullptr
//         return nullptr;
//     }
// }
// std::shared_ptr<cfg::map_struct>* test_findMapVar(struct cfg &myCfg, std::vector<std::string>&uri_keys)
// {
//     int key_idx = 0;
//     if ((uri_keys.size() > 1) && (uri_keys[0].size() == 0)) key_idx = 1;
//     if ((int)uri_keys.size() < key_idx+3) return nullptr;
//     printf(" >>>>>>>>>>>>> <%s> comp >> size %d <%s> \n"
//                             , __func__
//                             , (int)uri_keys.size()
//                             , uri_keys[key_idx].c_str()
//                             );


//     if (myCfg.itemMap.find(uri_keys[key_idx]) != myCfg.itemMap.end()) {
//         auto myComp = myCfg.itemMap[uri_keys[key_idx]];
//         printf(" >>>>>>>>>>>>> <%s> comp >> <%s>  reg <%s>\n"
//                             , __func__
//                             , uri_keys[key_idx].c_str()
//                             , uri_keys[key_idx+1].c_str()
//                             );

//         if (myComp.find(uri_keys[key_idx+1]) != myComp.end()) {
//             auto myReg = myComp[uri_keys[key_idx+1]];
//             if (myReg.find(uri_keys[key_idx+2]) != myReg.end()) {
//                 return &myReg[uri_keys[key_idx+2]];
...
bool test_fims_connect(struct cfg &myCfg, bool debug)
{
    ...
    const auto body = fmt::format("hello from modbus_client {}", name);
    ...
    NEW_FPS_ERROR_PRINT("client \"{}\": received a message [{}]\n", name, config_msg->body);
...
//using MapStructXPointer = std::shared_ptr<cfg::map_struct>;
using MapStructPointer = cfg::map_struct*;
...
//     // gcom_config_any
//     auto result = test_findMapVar(myCfg, uri_keys);


//     if(result)
//         std::cout << " var found " << result->id << "\n";
//     else
...
int main(const int argc, const char* argv[]) noexcept {
    ...
    if(cmd == "test")
    {
        ...
        std::cout << "test_points  <configs/client/gcom_test_client.json> <decode>                  : test all points in a config" << std::endl;
        std::cout << "test_point_type  <Discrete_Inputs> <decode>                                   : test all points of a given type " << std::endl;
        ...
        std::cout << "test_iopoint  <go>                                                            : test iopoint single point checker" << std::endl;
        std::cout << "    build/release/gcom_modbus_test test_iopoint 172.17.0.3 502 poll 1 Discrete_Input 395 1 23 4"<< std::endl;
        
        std::cout << "test_iopoint_multi  <go>                                                      : test new_iopoint multi optoons" << std::endl;
        std::cout << "    build/release/gcom_modbus_test test_iopoint 172.17.0.3 502 poll 1 Discrete_Input 395 1 23 4"<< std::endl;
        ...
        std::cout << "test_bad_regs  simple fims bad regs test                                       : test for bad regs"  << std::endl;
        std::cout << "test_decode  basic test decode to any                                          : test decode"  << std::endl;

        std::cout << "test_cfg_pub  <config_file>                                                    : test pub ( no threads)"  << std::endl;
    ...
    if(cmd == "test_fims")
    {
        ...
        if(debug) std::cout << "test_fims <file>  running :"<< std::endl;
        // std::map<std::string, std::any> gcom_map;
    ...
    if(cmd == "test_cfg")
    {
        ...
        // now test the body and the uri
        // test_uri_body(myCfg, "/components/comp_sel_2440/door_latch", "set", nullptr, nullptr, nullptr,"true");
        // // test_uri_body(myCfg, "/components/comp_sel_2440/door_latch", "set", nullptr, nullptr, nullptr,"1");
        // // test_uri_body(myCfg, "/components/comp_sel_2440/door_latch", "set", nullptr, nullptr, nullptr,"false");
        // // test_uri_body(myCfg, "/components/comp_sel_2440/door_latch", "set", nullptr, nullptr, nullptr,"0");
        // // test_uri_body(myCfg, "/components/comp_sel_2440", "set", nullptr, nullptr, nullptr,"{\"door_latch\":true,\"fire_relay\":0,\"some_float\":2.3,\"some_string\":\"a string\",\"some_value\":{\"value\":2345}}");
        // test_uri_body(myCfg, "/components/comp_sel_2440/hold_2_2", "set", nullptr, nullptr, nullptr,"{\"value\":1234}");
        // test_uri_body(myCfg, "/components/comp_sel_2440/hold_1_4", "set", nullptr, nullptr, nullptr,"{\"value\":123400}");
        ...
        test_uri_body(myCfg, "/components/comp_sel_2440/shold_1_4", "set", nullptr, nullptr, nullptr,"{\"value\":-500.67}");

        test_uri_body(myCfg, "/components/comp_sel_2440/fhold_1_2", "set", nullptr, nullptr, nullptr,"{\"value\":500.34}");
        test_uri_body(myCfg, "/components/comp_sel_2440/fhold_1_4", "set", nullptr, nullptr, nullptr,"{\"value\":-600.67}");
        test_uri_body(myCfg, "/components/comp_sel_2440/fshold_1_4", "set", nullptr, nullptr, nullptr,"{\"value\":-600.67}");
        ...
        test_uri_body(myCfg, "/components/comp_sel_2440/scale_1_1/_force", "set", nullptr, nullptr, nullptr,"{\"value\":300}");

        test_uri_body(myCfg, "/components/comp_sel_2440/scale_1_1", "set", nullptr, nullptr, nullptr,"{\"value\":100}");
        test_uri_body(myCfg, "/components/comp_sel_2440/scale_1_1/_unforce", "set", nullptr, nullptr, nullptr,"{\"value\":150}");
        ...
        // TODO bit mask bypass


        test_uri_body(myCfg, "/components/comp_sel_2440", "set", nullptr, nullptr, nullptr,"{\"multi_1\":1,\"multi_2\":2,\"multi_3\":3,\"multi_4\":4}");


        test_uri_body(myCfg, "/components/comp_sel_2440/multi_1", "get", nullptr, nullptr, nullptr,"{\"value\":0}");

        //test_uri_body(myCfg, "/components/comp_sel_2440", "set", "{\"hold_1_1\":2340}");
        //test_uri_body(myCfg, "/components/comp_sel_2440/hold_1_2", "set", "2345");
        //test_uri_body(myCfg, "/components/comp_sel_2440/hold_1_4", "set", "23456");
...
//    threadControl.responseThread.join();
    ...
    if (argc > 3)
    {
        ...
        if(cmd == "test_iopoint_multi")
        {
            ...
            test_io_point_multi(argv[2], atoi(argv[3]), 2, oper, device_id, reg_type, offset, value, num_threads, myCfg, debug);
        ...
        if(cmd == "test_iopoint")
        {
            ...
            if(debug) std::cout << "test_io <ip> <port> <set|pub|get> <device_id> <Holding|Coil> <offset> <num_regs> <value> <num_threads> <debug>" << std::endl;
             // in gcom_config_any
            gcom_load_cfg_file(gcom_map, filename, myCfg, false);

            test_io_point_single(argv[2], atoi(argv[3]), 2, oper, device_id, reg_type, offset, num_regs, value, num_threads, myCfg, debug);
