...
public:
    ...
    void sync () {  
        auto itime =  std::chrono::milliseconds(interval);
        auto remainingTime  = itime -
        (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()) % interval);
        if (remainingTime.count() <= itime.count() / 2) {
            synctime = itime.count() - remainingTime.count();
        }
    }
public:
    std::string name;
