...
network_root_queue = queue.Queue()
...
def run_ping_sweep():
    ...
    global frame_counter, network_root
    while True:
        # Check for and apply a new network root if available
        try:
            new_network_root = network_root_queue.get_nowait()
            network_root = new_network_root
            print(f"Network root updated to: {network_root}")
        except queue.Empty:
            pass
...
class MyHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif path == '/update_network':
            if self.command == 'POST':
                content_length = int(self.headers['Content-Length'])
                post_data = self.rfile.read(content_length).decode('utf-8')
                new_root = post_data.split('=')[1]
                
                # Validate the new root (e.g., regex check for "a.b.c")
                # For simplicity, we'll assume valid input for this patch
                
                network_root_queue.put(new_root + ".")
    ...
    def serve_main_page(self):
        ...
        <form method="POST" action="/update_network">
            <label for="network_root">Change Network Root:</label>
            <input type="text" id="network_root" name="network_root" value="{network_root[:-1]}">
