...
class MyHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def serve_main_page(self):
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT ip_address FROM ping_results ORDER BY ip_address")
        ips = [row[0] for row in cursor.fetchall()]
        
        html = "<h1>Ping Dashboard</h1>"
        html += "<h2>Select a Host to View Ping History</h2>"
        html += "<ul>"
        for ip in ips:
            html += f'<li><a href="/chart?ip={ip}">{ip}</a></li>'
        html += "</ul>"
    ...
    def serve_chart_page(self, ip_address):
        ...
        ping_times = [row[1] for row in data]
...
if __name__ == '__main__':
    ...
    # Start the threads
    scanner_thread.daemon = True # Allows the program to exit even if this thread is running
