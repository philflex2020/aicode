...
def load_limits_def_from_json(json_path: str, verbose: bool = False):
    ...
    ldefs = data.get("limits_def", [])
    # print(ldefs)
    # defs_len += len(ldefs)
    for ldef in ldefs:
         tab_name=ldef.get("table", "")
         print("table_name :", tab_name)  
         tab_items=ldef.get("items",[])
         for ld in tab_items:
            name=ld.get("name", ""),
    ...
    if verbose:
        print(f"[load_limits_def_from_json] Loaded {defs_len} limits_def entries.")
