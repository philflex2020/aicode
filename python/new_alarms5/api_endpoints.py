...
# Define the individual limit item model
limits_def_item_model = ns.model('LimitsDefItem', {
    'table': fields.String,
    'name': fields.String,
    'sm_name': fields.String,
    'reg_type': fields.String,
    'offset': fields.String,
    'num': fields.Integer,
    'write_data': fields.List(fields.Raw),
    'read_data': fields.List(fields.Raw),
})

# Define the grouped table model
limits_def_group_model = ns.model('LimitsDefGroup', {
    'table': fields.String,
    'items': fields.List(fields.Nested(limits_def_item_model)),
})
...
alarm_config_model = ns.model('AlarmConfig', {
    ...
    'limits_def': fields.List(fields.Nested(limits_def_group_model)),
})
...
#         limits_defs = LimitsDef.query.all()

#         # Group by table_name
#         grouped_limits = defaultdict(list)
#         for ld in limits_defs:
#             grouped_limits[ld.table_name].append(ld.to_dict())

#         # Build nested list structure
#         limits_def_nested = []
#         for table_name, items in grouped_limits.items():
#             limits_def_nested.append({
#                 'table': table_name,
#                 'items': items
#             })
...
class AlarmConfig(Resource):
    ...
    def get(self):
        ...
        for ld in limits_defs:
            print(f"LimitsDef record: id={ld.id}, table_name={ld.table_name}, name={ld.name}, sm_name={ld.sm_name}, reg_type={ld.reg_type}, offset={ld.offset}, num={ld.num} , write_data={ld.write_data}")

        print(f"limits_defs type: {type(limits_defs)}")
        for i, ld in enumerate(limits_defs):
            print(f"limits_defs[{i}] type: {type(ld)}")
            print(f"limits_defs[{i}] raw data: {ld}")


        # Group LimitsDef by table_name
        grouped_limits = defaultdict(list)
        for ld in limits_defs:
            grouped_limits[ld.table_name].append(ld.to_dict())

        # Build nested list structure for limits_def
        limits_def_nested = []
        for table_name, items in grouped_limits.items():
            limits_def_nested.append({
                'table': table_name,
        ...
        # Group LimitsValues by limits_structure
        limits_by_structure = {}
        for lv in limits_vals:
            limits_by_structure.setdefault(lv.limits_structure, []).append(lv.to_dict())
        ...
        # Enrich alarm definitions with limits_def field if needed
        enriched_alarm_defs = []
        for ad in alarm_defs:
            ad_dict = ad.to_dict()
            # Optionally attach limits_def list for this alarm's limits_structure
            # ad_dict['limits_def'] = limits_by_structure.get(ad.limits_structure, [])
            enriched_alarm_defs.append(ad_dict)

        return {
            'limits_def': limits_def_nested,
            'alarm_definitions': enriched_alarm_defs,
            'alarm_level_actions': prepared_alarm_levels,
            'limits_values': prepared_limits_vals
        }
    ...
    def post(self):
        ...
        try:
            ...
            # STEP 4: Add limits_def from UI (preserves user edits)
            provided_limits_def = data.get('limits_def', [])
            print(f"[/config POST] Received {len(provided_limits_def)} limits_def table groups")

            for table_group in provided_limits_def:
                table_name = table_group.get('table')
                items = table_group.get('items', [])
                for item in items:
                    # Remove 'table' key from item dict to avoid conflict with model attribute
                    item_clean = {k: v for k, v in item.items() if k != 'table'}
                    # Add table_name explicitly
                    item_clean['table_name'] = table_name
                    ld = LimitsDef(**item_clean)
                    db.session.add(ld)
...
class ExportJSON(Resource):
    ...
    def post(self):
        """Export alarm configuration JSON file as sent by frontend"""
        data = request.get_json()
        ...
        if not config:
            return {'message': 'No config data provided'}, 400
        ...
        try:
            ...
            # Write the posted config JSON directly to file
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(config, f, indent=2)

            return {'message': f'Exported JSON to {file_path}'}, 200
