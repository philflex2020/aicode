...
# New flattened import structure for Pymodbus 3.x
from pymodbus.server import StartAsyncTcpServer, ModbusDeviceIdentification
from pymodbus.datastore import ModbusSequentialDataBlock, ModbusSlaveContext, ModbusServerContext

async def run_pcs_sim(name, port, map_start, map_size):
    # Initialize Holding Registers (hr) with zeros
    # 3.x prefers a single list as the first argument
    data = [0] * map_size
    block = ModbusSequentialDataBlock(map_start, data)
    ...
    # Standard server call for 3.12.0
