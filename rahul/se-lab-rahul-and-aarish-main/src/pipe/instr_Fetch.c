...
static void fix_instr_aliases(uint32_t insnbits, opcode_t *op)
{
  ...
  if (*op == OP_ADDS_RR)
  {
    uint32_t Rn = bitfield_u32(insnbits, 5, 5); // bits 5-9
    if (Rn == 31)
    {                  // If Rn is XZR
  ...
  if (*op == OP_ANDS_RR)
  {
    uint32_t Rn = bitfield_u32(insnbits, 5, 5);
    if (Rn == 31)
    {                  // If Rn is XZR
...
comb_logic_t fetch_instr(f_instr_impl_t *in, d_instr_impl_t *out)
{
  ...
  select_PC(in->pred_PC, X_out->op, X_out->val_a, M_in->seq_succ_PC, M_out->op, M_out->cond_holds,
