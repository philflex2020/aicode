...
status = {s["name"]: {"status": "init"} for s in config if not s.get("disabled", False)}
rx_prev = {}
tx_prev = {}
time_prev = {}
lock = threading.Lock()
...
def poll_system(sysinfo):
    ...
    while True:
        data = {"name": name, "status": "unreachable"}
        try:
            ...
            for iface in sysinfo.get("interfaces", []):
                ...
                # Calculate MB/s
                last_t = time_prev.get(name, now)
                delta_t = max(now - last_t, 1e-3)

                rx_diff = rx - rx_prev.get(name, rx)
                tx_diff = tx - tx_prev.get(name, tx)

                rx_mbps = round(rx_diff / delta_t / 1024 / 1024, 2)
                tx_mbps = round(tx_diff / delta_t / 1024 / 1024, 2)

                data["net_rx_mbps"] = rx_mbps * 10
                data["net_tx_mbps"] = tx_mbps * 10
...
# Launch polling threads
for sysinfo in config:
    if sysinfo.get("disabled", False):
        continue
