...
int main(int argc, char** argv) {
    int option;
    TimerList tl;
    //std::thread tlThread = std::thread(tl.timerManagementThread, std::ref(tl));
    ...
    int p1 = 1;
    int p2 = 2;
    tl.add("timer1 test", 0, 500, 200, exampleCallback, &p1);


    tl.addOrModifyTimer("timer2", std::chrono::milliseconds(0), std::chrono::milliseconds(1000),
                                    std::chrono::milliseconds(500), exampleCallback, &p2);
    tl.showTimers();
