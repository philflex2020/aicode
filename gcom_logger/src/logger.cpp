...
namespace Logging
{
    ...
    bool to_file = true;
    std::string event_logger_name = "dnp3";
    ...
    EventLogger logger{event_logger_name, 64};
    ...
    void Init(std::string module, int argc, char** argv)
    {
        ...
        if (to_file)
        {
            logger = EventLogger(module.c_str(), 1);
            logger.set_level(severity_threshold); // log everything
            logger.flush_on(severity_threshold); // flush after every log
            logger.setPattern("{\"time\": \"%H:%M:%S\", \"date\": \"%D\", \"level\": \"%l\", %v}"); // This pattern is NEEDED site_controller is pushing json.
        }
     ...
     */
    void log_msg(spdlog::level::level_enum severity, std::tuple<std::string,std::string,std::string> pre, std::string msg_stripped, std::string original_msg, std::string post)
    {
        ...
        if (to_console)
            console->log(severity, std::get<1>(pre)+original_msg+redundant_msg); // pre<1> = console output
        if (to_file){ // log the message, then dump it to file
            logger.log(severity, std::get<0>(pre)+msg_stripped+redundant_msg+post); // pre<2> = log file output
            std::string dirAndFile = fmt::format("{}/{}.{}", log_dir, std::get<2>(pre), "txt"); // pre<2> = function name
     ...
     */
    std::tuple<std::string,std::string,std::string> pre_string( const char * file, const char* func, const int line){ 
        //msg
        ...
        snprintf (consoleBuf, 500, "\n\tCaller: %s:%d\n\tFunc: %s\n\tMessage: ", trim.c_str(), line, func);

        return std::tuple<std::string, std::string, std::string>(std::string(msgBuf), std::string(consoleBuf), std::string(func));
    }
