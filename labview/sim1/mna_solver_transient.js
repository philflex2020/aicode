...
let voltageSourceIndex = 0;
let inductorIndexOffset = elements.filter(e => e.type === 'voltage').length;
...
for (let step = 0; step < steps; step++) {
  ...
  elements.forEach((el, idx) => {
    ...
    if (el.type === 'resistor') {
      ...
      const g = dt / el.value;
      const row = n + inductorIndexOffset + indIdx;
  ...
  elements.forEach((el) => {
    if (el.type === 'capacitor') {
      const v1 = el.from === '0' ? 0 : V[nodeIndex[el.from] - 1][0];
      const v2 = el.to === '0' ? 0 : V[nodeIndex[el.to] - 1][0];
      el.state.voltage = v1 - v2;
    } else if (el.type === 'inductor') {
      const row = n + inductorIndexOffset + (--indIdx);
      el.state.current = V[row][0];
    }
  });
