...
for (let step = 0; step < steps; step++) {
  ...
  // ✅ Safe matrix condition estimate
  let cond = 0;
  try {
    const G_inv = math.inv(G);
    const normG = math.norm(G, 'fro');
    const normInv = math.norm(G_inv, 'fro');
    cond = normG * normInv;
  } catch (err) {
    console.warn(`⚠️ Step ${step}: Matrix inversion failed (singular?)`);
    continue;
  }

  if (!Number.isFinite(cond) || cond > 1e12) {
    console.warn(`⚠️ Step ${step}: Ill-conditioned matrix (cond=${cond.toExponential()})`);
    continue;
  }
  console.log("G matrix at step", step);
  console.log(G.toString());
  console.log("I vector at step", step);
