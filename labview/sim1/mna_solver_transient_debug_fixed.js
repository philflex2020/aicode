...
const elements = [
  ...
  { type: 'resistor', from: '3', to: '0', value: 100 }  // series resistor to ground
];
...
for (let step = 0; step < steps; step++) {
  ...
  try {
    ...
    cond = normG * normInv;
    console.log(`🔍 Step ${step} | Condition number: ${cond.toExponential()}`);
  } catch (err) {
    console.warn(`⚠️ Step ${step}: Matrix inversion failed`);
...
${I.toString()}`);
  ...
  if (!Number.isFinite(cond) || cond > 1e12) {
    console.warn(`⚠️ Step ${step}: Matrix is ill-conditioned`);
    continue;
  }
