...
user = "root"
passwd = "mynewsystem"
server_prog = "modbus_server"
client_prog = "modbus_client"
# Client and Server container names
client_container_name = "172.17.0.2"
server_container_name = "172.17.0.3"
timeout = 10
testdir = "test2"

def run_program_ssh_with_password(host, username, password, json_file, program, log_file):
    # Copy json file to the tmp directory on the host
    jfile="/tmp/modbus.json"
    lfile="/tmp/modbus.log"
    command = f"scp {json_file} {username}@{host}:{jfile}"
    print(command)
    print(log_file)
    #subprocess.run(command, shell=True, check=True, input=f"{password}\n", text=True)
    inpwd = bytes(f"{password}\n",'utf-8')
    print(inpwd)
    subprocess.run(command, shell=True, check=True, input=inpwd)

    # Run the program on the host for 10 seconds using timeout, collecting stdout and stderr into the log file
    #command = f"ssh {username}@{host} '(timeout {timeout} {program} {jfile}) &> {log_file}'"
    command = f"ssh {username}@{host} 'timeout {timeout} {program} {jfile} > {lfile} 2>&1&'"
    print(command)
    #subprocess.run(command, shell=True, check=True, input=f"{password}\n", text=True)
    subprocess.run(command, shell=True, check=True, input=inpwd)

# Function to collect log files from remote host with password authentication
def collect_logs_ssh_with_password(host, username, password, log_dir):
    lfile="/tmp/modbus.log"
    inpwd = bytes(f"{password}\n",'utf-8')
    ...
    command = f"scp {username}@{host}:{lfile} {os.path.join(log_dir, 'stdout_stderr.log')}"
    #subprocess.run(command, shell=True, check=True, input=f"{password}\n", text=True)
    subprocess.run(command, shell=True, check=True, input=inpwd)
...
def run_program_ssh(host, json_file, program, log_file):
    command = f"scp {json_file} user@{host}:/tmp/client.json"
    subprocess.run(command, shell=True, check=True)

    command = f"ssh user@{host} '(timeout 10 {program}) &> {log_file}'"
    subprocess.run(command, shell=True, check=True)

# Function to collect log files from remote host
def collect_logs_ssh(host, log_dir):
    time.sleep(10)  # Wait for 10 seconds to ensure logs are generated

    command = f"scp user@{host}:/tmp/stdout_stderr.log {os.path.join(log_dir, 'stdout_stderr.log')}"
    subprocess.run(command, shell=True, check=True)

# Main function to process each directory
def process_directory(test_dir, client_json, server_json, client_host, server_host):
    ...
    # Process server
    run_program_ssh_with_password(server_host, user, passwd, server_json, server_prog, os.path.join(server_log_dir, "stdout_stderr.log"))

    # Process client
    run_program_ssh_with_password(client_host, user, passwd, client_json, client_prog, os.path.join(client_log_dir, "stdout_stderr.log"))

    time.sleep(10)  # Wait for 10 seconds to ensure logs are generated

    # Wait for both processes to finish and collect log files
    collect_logs_ssh_with_password(client_host, user, passwd, client_log_dir)
    collect_logs_ssh_with_password(server_host, user, passwd, server_log_dir)
...
if __name__ == "__main__":
    # Path containing the subdirectories to process
    base_path = "test2"

    # List of directories to process
    #directories = ["path/to/dir1", "path/to/dir2", "path/to/dir3"]
    directories = get_subdirectories(base_path)


    test_results = []
    for dir_path in get_subdirectories(base_path):
