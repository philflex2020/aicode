...
def run_program_ssh_with_password(host, username, password, json_file, program, log_file):
    ...
    command = f"scp {json_file} {username}@{host}:/tmp/client.json"
    ...
    command = f"ssh {username}@{host} '(timeout 10 {program}) &> {log_file}'"
    subprocess.run(command, shell=True, check=True, input=f"{password}\n", text=True)
...
def collect_logs_ssh_with_password(host, username, password, log_dir):
    ...
    command = f"scp {username}@{host}:/tmp/stdout_stderr.log {os.path.join(log_dir, 'stdout_stderr.log')}"
    subprocess.run(command, shell=True, check=True, input=f"{password}\n", text=True)
...
def process_directory(test_dir, client_json, server_json, client_host, server_host):
    ...
    # Process client
    run_program_ssh_with_password(client_host, user, passwd, client_json, "xxx_client", os.path.join(client_log_dir, "stdout_stderr.log"))

    # Process server
    run_program_ssh_with_password(server_host, user, passwd, server_json, "xxx_server", os.path.join(server_log_dir, "stdout_stderr.log"))
