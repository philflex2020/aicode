...
set_objects = {float(key): value['Body'] for key, value in set_data.items() if value['Method'] == 'set'}
...
for set_time, set_obj in set_objects.items():
    ...
    for pub_time_str, pub_value in pub_items:
        ...
        if match_count < 2:
            ...
            if match_found:
                print(f"The objects at 'set' timestamp {set_time} are reflected in the 'pub' message at timestamp {pub_time}")
            else:
                print(f"The objects at 'set' timestamp {set_time} are not reflected in the 'pub' message at timestamp {pub_time}")
