...
extern "C" {
    void* initFunc(VarMapUtils *vm, varsmap& vmap, asset_manager *am, char *tclass , char *name, char *func, char *aV);
...
bool parseIncludeString(threadDataMaps &resultMap, const std::string& s) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (word == "amap") {
            std::string structName;
            lineStream >> structName;
            std::cout <<" found amap " << structName << std::endl;
        }
        if (word == "dtrans") {
            std::string structName;
            lineStream >> structName;
            std::cout <<" found dtrans " << structName << std::endl;
        }
...
void* loadCompFunc(VarMapUtils *vm, varsmap& vmap, asset_manager *am, char *tclass , char *name, char *fname, char *func, char *aV)
{
    std::string filename(fname);
    std::string fdir("/usr/local/lib/ess/");
    std::string fext(".so");
    fdir += filename;
    fdir += fext;
    FPS_PRINT_INFO("     >>> running {}  for fname [{}] ", __func__, fdir);
    void* handle = dlopen(fdir.c_str(), RTLD_LAZY);
    if (!handle) {
        std::cerr << "Error loading shared object: " << dlerror() << std::endl;
        return nullptr;
    }
    typedef void (*InitFunction)(VarMapUtils *vm, varsmap& vmap, asset_manager *am, char *tclass , char *name, char *func, char *aV);
    InitFunction initFunc = reinterpret_cast<InitFunction>(dlsym(handle, "initFunc"));
    if (!initFunc) {
        std::cerr << "Error getting the init function: " << dlerror() << std::endl;
        dlclose(handle);
        return nullptr;
    }
    initFunc(vm, vmap, am, tclass, name, func, aV);
    dlclose(handle);
    return nullptr;
}

// TODO ge the func from the class 
void setup_compFunc(VarMapUtils *vm, varsmap& vmap, asset_manager *am, char *tclass , char *name, char *fname, char *func, char *aV)
{
    ...
    void *compFunc = vm->getFunc(vmap, tclass, func);
    if (!compFunc)
    {
        loadCompFunc(vm, vmap, am, tclass, name, fname, func, aV);
        //compFunc = (void*)testcompFunc;
    }
    compFunc = vm->getFunc(vmap, tclass, func);
    if (!compFunc)
    {

        FPS_PRINT_INFO(" {}    >>> sadly using default testcompFunc ", __func__);
        compFunc = (void*)testcompFunc;
    }
...
//            makeEssThreadInst(cname, name);
        ...
        if (cmd == "setup_compFunc") {
            ...
            char *fname = aV->getcParam("fname");
            char *aVname = aV->getcParam("aV");
            if(cname && name && func && aVname)
            {
                setup_compFunc (vm , vmap, am, cname, name, fname, func , aVname);
            }
        }
    }
    return 0;
...
fims_send -m get -r /$$ -u /ess/full/system/functions | jq
